// @ts-nocheck
// Generated from Creator 2.4.8 by tools/migrate-njpdk-models.mjs.
// Legacy method bodies are preserved; only the BaseClass container is replaced.
export type LegacyNJPdkAppContext = Record<string, any>;

export class LegacyNJPdkRoomManager {
    public constructor(private readonly app: LegacyNJPdkAppContext) {
	...
	public Init(){
		...
		//比赛分通知
		this.NetManager.RegNetPack("SNJPDK_SportsPointNotEnough", this.OnPack_SportsPointNotEnough, this);
		this.NetManager.RegNetPack("SNJPDK_SportsPointEnough", this.OnPack_SportsPointEnough, this);
	...
	public OnPack_GetRoomInfo(serverPack){
		...
		// 独立子工程退出后会重建 JS 运行时；统一工程中的 SceneManager 是单例，
		// 其 sceneName 可能仍停留在 njpdkScene。必须同时核对 Cocos 当前真实场景，
		// 否则第二次进入会误走 ShowForm，拿旧的已销毁节点并永久停在进度条。
		let isActualGameScene = this.app.__isActualGameScene();
		if (this.app[this.app.subGameName + "_SceneManager"]().GetSceneType()!="njpdkScene" || !isActualGameScene) 
		{
			this.app[this.app.subGameName + "_SceneManager"]().LoadScene("njpdkScene");
			console.log("0000进入场景！内江跑得快！！！！！！！！！！！！！！！")
		}
		else{
			console.log("1111进入场景！内江跑得快！！！！！！！！！！！！！！！");
			this.app[this.app.subGameName + "_FormManager"]().ShowForm('game/NJPDK/UINJPDK_Play');
		}
	...
	public OnPack_SportsPointNotEnough(serverPack){
		this.app[this.app.subGameName + "Client"].OnEvent("SportsPointNotEnough", serverPack);
	}
	public OnPack_SportsPointEnough(serverPack){
		this.app[this.app.subGameName + "Client"].OnEvent("SportsPointEnough", serverPack);
	}
	//玩家的比赛分在游戏外被改变通知
	public OnPack_SportsPointChange(serverPack){
		this.Room.OnSportsPointChange(serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent("RoomSportsPointChange", serverPack);
	}
	//记牌器通知
	...
	public SendCreateRoom(sendPack){
		console.log('创建房间',sendPack)
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"CreateRoom", sendPack);
	}
	//获取房间信息
	public SendGetRoomInfo(roomID, callback){
		console.log('SendGetRoomInfo begin njpdk')
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"GetRoomInfo", {"roomID":roomID}, callback);
	}
	//解散房间
	public SendDissolveRoom(roomID){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"DissolveRoom", {"roomID":roomID});
	}
	//位置发送准备状态
	public SendReady(roomID, posIndex){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"ReadyRoom", {"roomID":roomID, "posIndex":posIndex});
	}
	//发送取消准备状态
	public SendUnReady(roomID, posIndex){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"UnReadyRoom", {"roomID":roomID, "posIndex":posIndex});
	}
	//房主T人
    public SendKickPosIndex(roomID, posIndex){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"KickRoom", {"roomID":roomID, "posIndex":posIndex});
	}
	//开始游戏
	public SendStartRoomGame(roomID){
		console.log('SendStartRoomGame roomID',roomID);
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"StartGame", {"roomID":roomID});
	}
	//发送继续游戏
	public SendContinueGame(roomID){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"ContinueGame", {"roomID":roomID});
	}
	//发送同意解散房间
	public SendDissolveRoomAgree(roomID){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"DissolveRoomAgree", {"roomID":roomID});
	}
	//发送拒绝解散房间
	public SendDissolveRoomRefuse(roomID){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"DissolveRoomRefuse", {"roomID":roomID});
	}
	//退出房间
	public SendExitRoom(roomID, pos){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"ExitRoom", {"roomID":roomID, "posIndex":pos});
	}
	//请求对局记录封包
	public SendRoomRecord(roomID){
        this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"RoomRecord", {"roomID":roomID});
    }
    //获取每一局的玩家记录
    ...
    public SendEndRoom(roomID){
    	this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"EndRoom", {"roomID":roomID});
	}
	public SendOpCard(sendPack){
    	this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"OpCard", sendPack);
	}
	public SendAddDouble(roomID, pos, addDouble){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"AddDouble", {"roomID":roomID,"pos":pos,"addDouble":addDouble});
	}
	public SendRoobDoor(roomID, pos, isRob){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"RobClose", {"roomID":roomID,"pos":pos,"robClose":isRob});
	}
	public SendOpenCard(roomID, pos, isOpen){
		this.NetManager.SendPack(this.app.subGameName+".C"+this.app.subGameName.toUpperCase()+"OpenCard", {"roomID":roomID,"pos":pos,"OpenCard":isOpen});
	}
}
