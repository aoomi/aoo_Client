import { assetManager, Button, Color, HorizontalTextAlignment, ImageAsset, Label, Node, Sprite, SpriteFrame, Texture2D, UITransform, VerticalTextAlignment } from 'cc';
import type { LegacyForm } from '../../../../../../core/runtime/ui/LegacyFormManager';
import type { LegacyNJPdkRuntime } from './LegacyNJPdkRuntime';
import { LegacyNJPdkGameLogic } from './logic/LegacyNJPdkGameLogic';
...
/** Creator 3 binding for the original UINJPDK_Play prefab and room operations. */
export class LegacyNJPdkPlayController {
    ...
    private readonly logic: LegacyNJPdkGameLogic;
    ...
    public constructor(
        ...
        this.logic = new LegacyNJPdkGameLogic({
    ...
    public onEvent(event: string, body: unknown): void {
        ...
        if (event === 'NJPDKSetEnd') this.showMessage('本局结束');
        if (event === 'RoomEnd') this.showMessage('房间牌局结束');
        if (event === 'NJPDK_DissolveRoom') {
            this.showMessage((body as any)?.ownnerForce ? '房主解散了房间' : '房间已解散');
            this.requestLeave('njpdk-room-dissolved');
        }
        this.refresh();
        this.refreshHeads();
    }
    ...
    private refresh(): void {
        ...
        this.text('roomInfo/labelBSF', cfg.clubId || cfg.unionId ? '亲友圈房间' : '好友房');
    ...
    private async renderHeads(): Promise<void> {
        if (!this.form) return;
        const manifest = await this.prefabs.loadManifest('legacy-ui/forms/njpdk/game/NJPDK/UIPublicHeadNJPDK.manifest');
        ...
        for (const key of Object.keys(players)) {
            ...
            head.name = `UIPublicHeadNJPDK${uiPos}`;
    ...
    private refreshHeads(): void {
        ...
        for (const [dataPos, head] of this.heads) {
            ...
            this.updateHeadImage(dataPos, head, String(player.headImageUrl ?? ''));
        }
    }

    private updateHeadImage(dataPos: number, head: Node, url: string): void {
        if (!url || this.headImageUrls.get(dataPos) === url) return;
        this.headImageUrls.set(dataPos, url);
        const extension = /\.png(?:$|\?)/i.test(url) ? '.png' : '.jpg';
        assetManager.loadRemote<ImageAsset>(url, { ext: extension }, (error, image) => {
            if (error || !image || !head.isValid || this.headImageUrls.get(dataPos) !== url) return;
