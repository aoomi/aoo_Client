// @ts-nocheck
// Generated from the Creator 2.4.8 NJPDK logic by tools/migrate-njpdk-logic.mjs.
// Method bodies intentionally remain byte-for-byte equivalent apart from the class wrapper.
export interface LegacyNJPdkRoomPolicy {
    GetRoomPaiXing(name: string): boolean;
}

export interface LegacyNJPdkRoomSetSource {
    GetHandCard(): number[];
}

export interface LegacyNJPdkGameLogicOptions {
    room?: LegacyNJPdkRoomPolicy;
    roomSet?: LegacyNJPdkRoomSetSource;
    onEvent?: (event: string) => void;
}

export class LegacyNJPdkGameLogic {
    public handCardList = [];
    ...
    public readonly Room: LegacyNJPdkRoomPolicy;
    public readonly RoomSet: LegacyNJPdkRoomSetSource;
    private readonly onEvent: (event: string) => void;

    public constructor(options: LegacyNJPdkGameLogicOptions = {}) {
    ...
    public TransformValueToC(pokers){
    ...
    public CheckSanDaiSiDai(tag,isSelectCard=true){ 
        ...
        if(tag == 5){
            ...
            //三带一不能出4根一样的end
            if(!this.Room.GetRoomPaiXing('SanDaiYi')){
                return false;
            }
        }
        else if(tag == 7){
            //没有三带二的玩法 不检测
            if(!this.Room.GetRoomPaiXing('SanDaiEr'))
            {
                return false;
            }
        }
            ...
            //没有三带一对的玩法 不检测
            if(!this.Room.GetRoomPaiXing('SanDaiYiDui')){
                return false;
            }
        }
        /*else if(tag == 8){
    ...
    public CheckSanDaiFeiJi(optype,isSelectCard=true){
        ...
        if(tag == 3){
            ...
            if(this.Room.GetRoomPaiXing('SanDaiYi') && optype==16){
            ...
            if(this.Room.GetRoomPaiXing('SanDaiYiDui') && optype==17){
            ...
            if(this.Room.GetRoomPaiXing('SanDaiEr') && optype==18){
    ...
    public GetZhaDanEx(array,isSelectCard){
        ...
        // if(this.Room.GetRoomPaiXing('SanDaiYi') == true){
    ...
    public GetSanDaiFeiJiTip(lastCardType,tag,isSelectCard=false){
        ...
        if(realPlane.length){
            if(this.Room.GetRoomPaiXing('SanDaiYi') && lastCardType==16){
                this.GetOtherCard(realPlane,realPlane[0].length/3,isSelectCard);
            }
            else if(this.Room.GetRoomPaiXing('SanDaiYiDui') && lastCardType==17){
                this.GetOtherCardDui(realPlane,(realPlane[0].length/3)*2,isSelectCard);
            }
            else if(this.Room.GetRoomPaiXing('SanDaiEr') && lastCardType==18){
    ...
    public GetSanDaiFeiJi(tag,isSelectCard=false){
        ...
        if(realPlane.length){
            if(this.Room.GetRoomPaiXing('SanDaiYi') && this.lastCardType==16){
                this.GetOtherCard(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
            }
            else if(this.Room.GetRoomPaiXing('SanDaiYiDui') && this.lastCardType==17){
                this.GetOtherCardDui(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
            }
            else if(this.Room.GetRoomPaiXing('SanDaiEr') && this.lastCardType==18){
                this.GetOtherCard(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
    ...
    public GetFeiJi(tag,isSelectCard=false){
        ...
        if(realPlane.length){
            if(tag==3){
                ...
                else if(this.Room.GetRoomPaiXing('SanDaiYiDui')){
                    this.GetOtherCardDui(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
                }
                else if(this.Room.GetRoomPaiXing('SanDaiEr')){
                    this.GetOtherCard(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
                }
            }else if(tag==4){
                this.GetOtherCard(realPlane, this.lastCardList.length - realPlane[0].length,isSelectCard);
