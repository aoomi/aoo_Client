...
export class LegacyScjymjRoomSet {
  ...
  public Init() {
		...
		for (let posID = 0; posID < this.ShareDefine.SCJYMJRoomJoinCount; posID++) {
			this.cacheSetPosDict[posID] = this.app.SCJYMJSetPos();
		}
  ...
  public OnSetRound(obj) {
		this.dataInfo['roundObj'] = obj;
  }
  ...
  public SetHead(obj) {
		this.dataInfo.heads = obj
  }

  public GetHead() {
		return this.dataInfo.heads;
  }
  public setPosListTing(arr) {
		this.dataInfo['setPosList'] = arr
  }
  ...
  public OnPosOpCard(serverPack) {
		...
		let clientPos = this.SCJYMJRoomPosMgr.GetClientPos();
		if (clientPos == pos) {
  ...
  public PreSetShouCard(pos, cardID) {
		...
		let num = this.ShareDefine.SCJYMJDingQue[dingQue];
