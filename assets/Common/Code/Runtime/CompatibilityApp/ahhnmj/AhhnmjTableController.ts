...
const modalPaths: Record<ModalName, string> = {
    chat: 'native-ui/game/ahhnmj/resources/ui/ahhnmj_UIChat',
    settings: 'native-ui/game/ahhnmj/resources/ui/ahhnmj_UISetting02',
    dissolve: 'native-ui/game/ahhnmj/resources/ui/ahhnmj_UIMessage02',
};

/** Binds migrated table/menu prefabs to their original AHHNMJ protocols. */
export class AhhnmjTableController {
    ...
    public constructor(
        ...
        private readonly runtime: AhhnmjRuntime,
        private readonly message: (text: string) => void,
    ) {
        this.bindTableButtons();
        this.root.on('legacy-ahhnmj-event', this.onRoomEvent, this);
    }

    public destroy(): void {
        this.disposed = true;
        this.root.off('legacy-ahhnmj-event', this.onRoomEvent, this);
        this.closeModal();
    }
    ...
    private bindTableButtons(): void {
        ...
        this.walk(this.root, (node) => {
            ...
            const jiaoZui = /^btn_jiaozui(\d+)$/.exec(node.name.toLowerCase());
            const nao = /^btn_nao(\d+)$/.exec(node.name.toLowerCase());
            if (jiaoZui) this.bind(node, () => { void this.runtime.chooseJiaoZui(Number(jiaoZui[1])).catch(this.report('叫嘴失败')); });
            else if (nao) this.bind(node, () => { void this.runtime.chooseNao(Number(nao[1])).catch(this.report('闹失败')); });
        });
    }
    ...
    private onRoomEvent(payload: unknown): void {
        ...
        if (event.endsWith('startvotedissolve') || event.endsWith('posdealvote')) {
            ...
            this.root.emit('legacy-ahhnmj-chat-message', packet.body);
        }
    }
    ...
    private bindSettings(modal: Node): void {
        ...
        for (const name of ['sd_yinyue', 'sd_yinxiao']) {
            ...
            slider?.node.on('slide', () => this.root.emit('legacy-ahhnmj-volume', { name, value: slider.progress }), this);
        }
        const viewButtons = ['btn_2d', 'btn_XY', 'btn_WZ', 'btn_YF'];
        for (const name of viewButtons) {
            this.bindNamed(modal, name, () => this.root.emit('legacy-ahhnmj-view-mode-request', name.slice(4).toLowerCase()));
        }
    }
