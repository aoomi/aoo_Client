...
export class LegacyAhhnmjRoomSet {
  ...
  public UpdatezhangZhuangNum(zhangZhuangNum) {
		this.dataInfo["zhangZhuangNum"] = zhangZhuangNum;
  }
  ...
  public PreSetShouCard(pos, cardID) {
		...
		if (handCard > 0 && handCard == cardID) {
			...
			if (handCard > 0) {
				...
				let dingQue = this.app[this.app.subGameName + "_PlayManager"]().GetDingQue(setPos.GetSetPosProperty("dingQue"));
				for (let i = 0; i < count; i++) {
					if (Math.floor(shouCard[i] / 1000) != dingQue) {
  ...
  public GetHuCardTypeInfo(selfPos) {
		...
		for (var pos in this.allSetPosDict) {
			...
			for (let index = 0; index < publicCount; index++) {
				...
				let cardIDList=[];
				// if(opType==this.ShareDefine.OpType_AnGang && selfPos != pos){
				// 	cardIDList=[];
				// }else{
					cardIDList = publicInfoList.slice(3, publicInfoList.length);
