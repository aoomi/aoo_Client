// @ts-nocheck
// Generated from the Creator 2.2.2 AHHNMJ room model; method bodies and protocol semantics are preserved.
export type LegacyAhhnmjAppContext = Record<string, any>;
export class LegacyAhhnmjRoomManager {
  public Log: (...args: any[]) => void = () => undefined;
  public ErrLog: (...args: any[]) => void = () => undefined;
  public constructor(private readonly app: LegacyAhhnmjAppContext) { this.Init(); }
  ...
  public Init() {
		...
		//叫嘴
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_JiaoZui", this.OnPack_JiaoZui, this);
		//闹嘴
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_Nao", this.OnPack_Nao, this);
  ...
  public OnPack_ChangeStatus(serverPack) {
	...
	//玩家叫嘴
  public OnPack_JiaoZui(serverPack) {
		console.log("玩家叫嘴 OnPack_JiaoZui", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_JiaoZui", serverPack);
  }
	//玩家闹嘴
  public OnPack_Nao(serverPack) {
		console.log("玩家闹嘴 OnPack_Nao", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_Nao", serverPack);
  }
	//局内消息表情
  ...
  public SendJiaoZui(jiaoZui) {
		if(!this.enterRoomID){
			this.ErrLog("SendJiaoZui not enterRoomID");
			return
		}
		let roomID = this.enterRoomID;
		let self = this;
        this.NetManager.requestV2(this.app.subGameName.toUpperCase() + ".C" + this.app.subGameName.toUpperCase() + "JiaoZui", {"roomID":roomID, "jiaoZui":jiaoZui},function(success){
        	console.log("JiaoZui请求成功 jiaoZui:", jiaoZui);
        },function(error){
        	self.ErrLog("JiaoZui.error:%s", error);
        });
  }
  public SendNao(nao) {
		if(!this.enterRoomID){
			this.ErrLog("SendNao not enterRoomID");
			return
		}
		let roomID = this.enterRoomID;
		let self = this;
        this.NetManager.requestV2(this.app.subGameName.toUpperCase() + ".C" + this.app.subGameName.toUpperCase() + "Nao", {"roomID":roomID, "nao":nao},function(success){
        	console.log("Nao请求成功 jiaoZui:", jiaoZui);
        },function(error){
        	self.ErrLog("Nao.error:%s", error);
        });
  }
