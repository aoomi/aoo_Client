...
import type { AhhnmjRuntime } from './AhhnmjRuntime';
import { AhhnmjPlayStateController, type AhhnmjPlaySnapshot } from './AhhnmjPlayStateController';
import type { AhhnmjViewMode } from './AhhnmjSceneBootstrap';
import { AhhnmjTableController } from './AhhnmjTableController';
...
const textureFolder: Record<AhhnmjViewMode, string> = {
...
/** Native Creator 3.8.8 binder shared by all four migrated AHHNMJ table prefabs. */
@ccclass('AhhnmjPlayView')
export class AhhnmjPlayView extends Component {
    private runtime: AhhnmjRuntime | null = null;
    private state: AhhnmjPlayStateController | null = null;
    private viewMode: AhhnmjViewMode = '2d';
    private generation = 0;
    private resultNode: Node | null = null;
    private table: AhhnmjTableController | null = null;

    public initialize(runtime: AhhnmjRuntime, viewMode: AhhnmjViewMode): void {
        this.runtime = runtime;
        this.viewMode = viewMode;
        this.state = new AhhnmjPlayStateController(runtime);
        this.table = new AhhnmjTableController(this.node, runtime, (message) => this.node.emit('legacy-ahhnmj-message', message));
        this.bindOperationButtons();
        this.node.on('legacy-ahhnmj-event', this.onRoomEvent, this);
    }
    ...
    protected override onDestroy(): void {
        this.generation += 1;
        this.node.off('legacy-ahhnmj-event', this.onRoomEvent, this);
    ...
    private async render(snapshot: AhhnmjPlaySnapshot): Promise<void> {
    ...
    private async showResult(phase: 'settled' | 'ended'): Promise<void> {
        ...
        const path = phase === 'ended'
            ? `native-ui/game/ahhnmj/resources/game/AHHNMJ/base/ahhnmj_UIMJResultDetail${suffix}`
            : `native-ui/game/ahhnmj/resources/game/AHHNMJ/base/ahhnmj_UIMJWinLost${suffix}`;
    ...
    private loadCardFrame(cardId: number): Promise<SpriteFrame> {
        ...
        const path = `legacy-ui/ahhnmj-source/${textureFolder[this.viewMode]}/hh_face_li_${name}/spriteFrame`;
