...
import { BamjSceneBootstrap, type BamjViewMode } from './BamjSceneBootstrap';

/** Native handoff coordinator for backend game 363 (Jiangxi Ji'an Anfu Mahjong). */
export class BamjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: BamjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 363 || handoff.gameName.toLowerCase() === 'bamj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: BamjViewMode = '2d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'bamj');
            const host = new Node('BAMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(BamjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-bamj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-bamj-room-ready', { roomId, handoff, viewMode });
