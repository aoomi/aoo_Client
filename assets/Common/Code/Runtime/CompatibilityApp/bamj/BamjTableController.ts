...
const modalPaths: Record<ModalName, string> = {
    chat: 'native-ui/game/bamj/resources/ui/bamj_UIChat',
    settings: 'native-ui/game/bamj/resources/ui/bamj_UISetting02',
    dissolve: 'native-ui/game/bamj/resources/ui/bamj_UIMessage02',
};

/** Binds migrated table/menu prefabs to their original BAMJ protocols. */
export class BamjTableController {
    ...
    public constructor(
        ...
        private readonly runtime: BamjRuntime,
        private readonly message: (text: string) => void,
    ) {
        this.bindTableButtons();
        this.root.on('legacy-bamj-event', this.onRoomEvent, this);
    }

    public destroy(): void {
        this.disposed = true;
        this.root.off('legacy-bamj-event', this.onRoomEvent, this);
        this.closeModal();
    }
    ...
    private onRoomEvent(payload: unknown): void {
        ...
        if (event.endsWith('startvotedissolve') || event.endsWith('posdealvote')) {
            ...
            this.root.emit('legacy-bamj-chat-message', packet.body);
        }
    }
    ...
    private bindSettings(modal: Node): void {
        ...
        for (const name of ['sd_yinyue', 'sd_yinxiao']) {
            ...
            slider?.node.on('slide', () => this.root.emit('legacy-bamj-volume', { name, value: slider.progress }), this);
        }
        const viewButtons = ['btn_2d', 'btn_XY', 'btn_WZ', 'btn_YF'];
        for (const name of viewButtons) {
            this.bindNamed(modal, name, () => this.root.emit('legacy-bamj-view-mode-request', name.slice(4).toLowerCase()));
        }
    }
