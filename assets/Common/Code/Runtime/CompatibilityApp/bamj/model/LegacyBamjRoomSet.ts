...
export class LegacyBamjRoomSet {
  ...
  public getHuangZhuang() {
		return this.dataInfo.huangZhuang || 0;
  }
  public get_dijiquan() {
		return this.dataInfo.quanShu || 0;
  }
  public get_jin1() {
		return this.dataInfo.jin || 0;
  }
  public get_jin2() {
  ...
  public InitSetInfo(setInfo) {
		...
		let cardRestSize = setInfo["cardRestSize"];
		if (!cardRestSize) {
			cardRestSize = 144 - setInfo["gangMoCnt"] - setInfo["normalMoCnt"];
		}
		this.dataInfo["cardRestSize"] = cardRestSize;
  }
  ...
  public OnStartRound(setRound) {
	...
	//指定位置获取卡牌
  public OnPosGetCard(pos, setPosInfo, normalMoCnt, gangMoCnt, cardRestSize) {
		let setPos = this.allSetPosDict[pos];
		if (!setPos) {
  ...
  public GetPosLianZhuang(pos) {
		let setPos = this.allSetPosDict[pos];
		return setPos.GetSetPosProperty("lianZhuang");
  }
  public GetPosJi(pos) {
		let setPos = this.allSetPosDict[pos];
		if (setPos.GetSetPosProperty("chong") == true) {
			return 0;
		}
		if (setPos.GetSetPosProperty("chong8") == true) {
			return 1;
		}
		if (setPos.GetSetPosProperty("ze") == true) {
			return 2;
		}
		if (setPos.GetSetPosProperty("ze8") == true) {
			return 3;
		}
		return -1;
  }
  ...
  public PreSetShouCard(pos, cardID) {
	...
	// 			//白板换成金排序
	// 			for (let i = 0; i < count; i++) {
	// 				if (Math.floor(oldShouCard[i] / 100) == 51) {
	// 					//梅
	// 					let yushu = oldShouCard[i] % 100;
	// 					let newCardID = Math.floor(jin1 / 100) * 100 + yushu + 4;
	// 					newShouCard2.push(newCardID);
	// 				} else if (Math.floor(oldShouCard[i] / 100) == 52) {
	// 					//兰
	// 					let yushu = oldShouCard[i] % 100;
	// 					let newCardID = Math.floor(jin1 / 100) * 100 + yushu + 4;
	// 					newShouCard2.push(newCardID);
	// 				} else if (Math.floor(oldShouCard[i] / 100) == 53) {
	// 					//竹
	// 					let yushu = oldShouCard[i] % 100;
	// 					let newCardID = Math.floor(jin1 / 100) * 100 + yushu + 4;
	// 					newShouCard2.push(newCardID);
	// 				} else if (Math.floor(oldShouCard[i] / 100) == 54) {
	// 					//菊
	// 					let yushu = oldShouCard[i] % 100;
	// 					let newCardID = Math.floor(jin1 / 100) * 100 + yushu + 4;
	// 					newShouCard2.push(newCardID);
	...
	// 					let newCardID2 = 5100 + yushu - 4;
	// 					newShouCard.push(newCardID2);
	// 				} else if (Math.floor(newShouCard2[i] / 100) == Math.floor(jin1 / 100)) {
	// 					//兰
	// 					let yushu = newShouCard2[i] % 100;
	// 					let newCardID2 = 5200 + yushu - 4;
	// 					newShouCard.push(newCardID2);
	// 				} else if (Math.floor(newShouCard2[i] / 100) == Math.floor(jin1 / 100)) {
	// 					//竹
	// 					let yushu = newShouCard2[i] % 100;
	// 					let newCardID2 = 5300 + yushu - 4;
	// 					newShouCard.push(newCardID2);
	// 				} else if (Math.floor(newShouCard2[i] / 100) == Math.floor(jin1 / 100)) {
	// 					//菊
	// 					let yushu = newShouCard2[i] % 100;
	// 					let newCardID2 = 5400 + yushu - 4;
	// 					newShouCard.push(newCardID2);
	// 				} else {
	...
	// 			//先抽出条，条》万》筒
	// 			for (let i = 0; i < count; i++) {
	// 				if (shouCard[i] > 2000 && shouCard[i] < 3000) {
	// 					newShouCard.push(shouCard[i]);
	// 					oldShouCard.Remove(shouCard[i]);
	// 				}
	// 			}
	// 			newShouCard.SortList();
	// 			count = oldShouCard.length;
	// 			//白板换成金排序
	// 			for (let i = 0; i < count; i++) {
	...
	// 					oldShouCard.Remove(shouCard[i]);
	// 				}
	// 				if (Math.floor(shouCard[i] / 100) > 10 && Math.floor(shouCard[i] / 100) < 20) {
	// 					wan.push(shouCard[i]);
	// 					oldShouCard.Remove(shouCard[i]);
	// 				}
	// 				if (Math.floor(shouCard[i] / 100) > 20 && Math.floor(shouCard[i] / 100) < 30) {
	// 					tiao.push(shouCard[i]);
	// 					oldShouCard.Remove(shouCard[i]);
	// 				}
	// 				if (Math.floor(shouCard[i] / 100) > 30 && Math.floor(shouCard[i] / 100) < 40) {
	// 					tong.push(shouCard[i]);
	// 					oldShouCard.Remove(shouCard[i]);
	// 				}
	...
	// 			tong.SortList();
	// 			tiao.SortList();
	// 			newShouCard = newShouCard.concat(tiao);
	// 			newShouCard = newShouCard.concat(wan);
	// 			newShouCard = newShouCard.concat(tong);
