...
import { AqmjSceneBootstrap, type AqmjViewMode } from './AqmjSceneBootstrap';

/** Native handoff coordinator for backend game 232 (Jiangxi Ji'an Anfu Mahjong). */
export class AqmjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AqmjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 232 || handoff.gameName.toLowerCase() === 'aqmj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AqmjViewMode = '2d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'aqmj');
            const host = new Node('AQMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AqmjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-aqmj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-aqmj-room-ready', { roomId, handoff, viewMode });
