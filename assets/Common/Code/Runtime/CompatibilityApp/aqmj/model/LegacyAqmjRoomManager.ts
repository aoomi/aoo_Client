// @ts-nocheck
// Generated from the Creator 2.2.2 AQMJ room model; method bodies and protocol semantics are preserved.
export type LegacyAqmjAppContext = Record<string, any>;
export class LegacyAqmjRoomManager {
  public Log: (...args: any[]) => void = () => undefined;
  public ErrLog: (...args: any[]) => void = () => undefined;
  public constructor(private readonly app: LegacyAqmjAppContext) { this.Init(); }
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_Zhang", this.OnPack_Zhang, this);
		//俱乐部积分通知
  ...
  public OnPack_Zhang(serverPack) {
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_Zhang", serverPack);
  }
  ...
  public OnPack_ChangeStatus(serverPack) {
		...
		roomSet.UpdatezhangZhuangNum(serverPack.zhangZhuangNum);
  ...
  public SendPosAction(cardID, opType, cardList = []) {
		...
		this.NetManager.requestV2(this.app.subGameName.toUpperCase() + ".C" + this.app.subGameName.toUpperCase() + "OpCard", sendPack, (data) => {
			...
			if (opType == this.ShareDefine.OpType_Out || opType == this.ShareDefine.OpType_BaoTing) {//打牌
				if (is3DShow == 0) {
  ...
  public SendPiao(piaoHua) {
		...
		this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "ZhangZhuang", {
			"roomID": this.enterRoomID,
			"zhang": piaoHua
		});
  }
