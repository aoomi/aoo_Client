...
import { AfmjSceneBootstrap, type AfmjViewMode } from './AfmjSceneBootstrap';

/** Native handoff coordinator for backend game 399 (Jiangxi Ji'an Anfu Mahjong). */
export class AfmjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AfmjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 399 || handoff.gameName.toLowerCase() === 'afmj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AfmjViewMode = '2d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'afmj');
            const host = new Node('AFMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AfmjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-afmj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-afmj-room-ready', { roomId, handoff, viewMode });
