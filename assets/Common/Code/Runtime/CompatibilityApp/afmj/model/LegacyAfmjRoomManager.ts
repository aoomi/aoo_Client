...
export class LegacyAfmjRoomManager {
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_PiaoHua", this.OnPack_PiaoHua, this);
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_MaiMa", this.OnPack_MaiMa, this);
		//报定
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_MingPai", this.OnPack_MingPai, this);
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_ChaoZhuang", this.OnPack_ChaoZhuang, this);	// 抄庄
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_XiaJing", this.OnPack_XiaJing, this);	// 上下翻精
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_SuiJing", this.OnPack_SuiJing, this);	// 随精
  ...
  public OnPack_SetPosCard(serverPack) {
	...
	//有玩家飘花通知
  public OnPack_PiaoHua(serverPack) {
		console.log("有玩家飘花通知 OnPack_PiaoHua", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_PiaoHua", serverPack);
  }
	//有玩家飘花通知
  public OnPack_MaiMa(serverPack) {
		console.log("有玩家买码通知 OnPack_MaiMa", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_MaiMa", serverPack);
  }
	//有玩家报定时通知
  public OnPack_MingPai(serverPack) {
		console.log("有玩家报定/明摆时通知 OnPack_MingPai", serverPack);

		let roomID = serverPack["roomID"];
		let setPos = serverPack["setPos"];
		this[this.app.subGameName.toUpperCase() + "Room"].GetRoomSet().MingPaiOpCard(serverPack);
		this[this.app.subGameName.toUpperCase() + "Room"].GetRoomSet().SetBaoDingPiaoHua(setPos);

		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_MingPai", serverPack);
  }
	//抄庄
  public OnPack_ChaoZhuang(serverPack) {
		console.log("抄庄 OnPack_ChaoZhuang", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_ChaoZhuang", serverPack);
  }
	// 上下翻精
  public OnPack_XiaJing(serverPack) {
		console.log("上下翻精 OnPack_XiaJing", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_XiaJing", serverPack);
  }
	// 随精
  public OnPack_SuiJing(serverPack) {
		console.log("随精 OnPack_SuiJing", serverPack);
		let pos = serverPack["pos"];
		let zhengJingCardId = serverPack["suizhengJingCardId"];
		let fuJingCardId = serverPack["suiFuJingCardId"];
		let roomSet = this[this.app.subGameName.toUpperCase() + "Room"].GetRoomSet();

		roomSet.SetSuiZhengJingCardId(pos, zhengJingCardId);		// 正精牌id
		roomSet.SetSuiFuJingCardId(pos, fuJingCardId);				// 副精牌id

		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_SuiJing", serverPack);
  }
  ...
  public SendPiao(piaoHua) {
	...
	// 随精 （选择本局精牌）
  public SendSuiJing(suizhengJingCardId) {
		console.log(" 随精 （选择本局精牌） SendSuiJing", suizhengJingCardId);
		this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "SuiJing", {
			"roomID": this.enterRoomID,
			"suizhengJingCardId": suizhengJingCardId
		});
  }
