...
import type { AfmjRuntime } from './AfmjRuntime';
import { AfmjPlayStateController, type AfmjPlaySnapshot } from './AfmjPlayStateController';
import type { AfmjViewMode } from './AfmjSceneBootstrap';
import { AfmjTableController } from './AfmjTableController';
...
const textureFolder: Record<AfmjViewMode, string> = {
...
/** Native Creator 3.8.8 binder shared by all four migrated AFMJ table prefabs. */
@ccclass('AfmjPlayView')
export class AfmjPlayView extends Component {
    private runtime: AfmjRuntime | null = null;
    private state: AfmjPlayStateController | null = null;
    private viewMode: AfmjViewMode = '2d';
    private generation = 0;
    private resultNode: Node | null = null;
    private table: AfmjTableController | null = null;

    public initialize(runtime: AfmjRuntime, viewMode: AfmjViewMode): void {
        this.runtime = runtime;
        this.viewMode = viewMode;
        this.state = new AfmjPlayStateController(runtime);
        this.table = new AfmjTableController(this.node, runtime, (message) => this.node.emit('legacy-afmj-message', message));
        this.bindOperationButtons();
        this.node.on('legacy-afmj-event', this.onRoomEvent, this);
    }
    ...
    protected override onDestroy(): void {
        this.generation += 1;
        this.node.off('legacy-afmj-event', this.onRoomEvent, this);
    ...
    private async render(snapshot: AfmjPlaySnapshot): Promise<void> {
    ...
    private async showResult(phase: 'settled' | 'ended'): Promise<void> {
        ...
        const path = phase === 'ended'
            ? `native-ui/game/afmj/resources/game/AFMJ/base/afmj_UIMJResultDetail${suffix}`
            : `native-ui/game/afmj/resources/game/AFMJ/base/afmj_UIMJWinLost${suffix}`;
    ...
    private loadCardFrame(cardId: number): Promise<SpriteFrame> {
        ...
        const path = `legacy-ui/afmj-source/${textureFolder[this.viewMode]}/hh_face_li_${name}/spriteFrame`;
