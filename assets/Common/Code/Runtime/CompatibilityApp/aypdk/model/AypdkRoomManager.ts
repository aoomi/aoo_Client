...
export class AypdkRoomManager {
	...
	public OnPack_GetRoomInfo(serverPack){
		...
		// 其 sceneName 可能仍停留在 aypdkScene。必须同时核对 Cocos 当前真实场景，
		// 否则第二次进入会误走 ShowForm，拿旧的已销毁节点并永久停在进度条。
		let isActualGameScene = this.app.__isActualGameScene();
		if (this.app[this.app.subGameName + "_SceneManager"]().GetSceneType()!="aypdkScene" || !isActualGameScene) 
		{
