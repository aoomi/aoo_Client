...
export class AypdkCardRenderer {
    ...
    public async create(card: number, parent: Node, selected: boolean): Promise<Node> {
        const manifest = await this.prefab.loadManifest('legacy-ui/forms/aypdk/game/AYPDK/cardPrefab.manifest');
    ...
    private frame(root: Node, childName: string, assetName: string | null): void {
        ...
        resources.load(`legacy-ui/assets/aypdk/texture/new_poker/${assetName}/spriteFrame`, SpriteFrame, (error, frame) => {
