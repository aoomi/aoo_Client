...
import type { AypdkShareController } from './AypdkShareController';

export class AypdkRecordController {
    ...
    public constructor(
        ...
        private readonly shareController: AypdkShareController,
    ...
    private render(): void {
        ...
        for (let index = 0; index < 4; index += 1) {
            ...
            this.text(`${root}/lb_ClubCent`, info.clubCent == null ? '' : `${Number(info.clubCent) > 0 ? '+' : ''}${info.clubCent}`);
        }
    }

    private async continueRoom(): Promise<void> {
        try {
            await this.runtime.request('aypdk.CAYPDKContinueEnterRoom', {});
