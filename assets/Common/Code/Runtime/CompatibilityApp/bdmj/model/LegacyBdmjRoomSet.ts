...
export class LegacyBdmjRoomSet {
  ...
  public Init() {
		...
		for (let posID = 0; posID < this.ShareDefine.BDMJRoomJoinCount; posID++) {
			this.cacheSetPosDict[posID] = this.app.BDMJSetPos();
		}
		this.OnReload();
  ...
  public OnPosOpCard(serverPack) {
		...
		let setPos = this.allSetPosDict[pos];
		setPos.kouPlaying=setPosInfo.kouPlaying;
		if (!setPos) {
		...
		let clientPos = this.BDMJRoomPosMgr.GetClientPos();
		if (clientPos == pos) {
