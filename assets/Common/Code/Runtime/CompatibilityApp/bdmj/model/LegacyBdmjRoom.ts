...
export class LegacyBdmjRoom {
  ...
  public OnReload() {
		...
		this.BDMJRoomPosMgr.OnReload();
		this.BDMJRoomSet.OnReload();
  }
  ...
  public OnInitRoomData(serverPack) {
		...
		this.BDMJRoomPosMgr.OnInitRoomPosData(roomPosInfoList);

		let setInfo = serverPack["set"];
		this.BDMJRoomSet.OnInitRoomSetData(setInfo);
		//初始化金
  ...
  public OnPosLeave(pos) {
		this.BDMJRoomPosMgr.OnPosLeave(pos);
		this.BDMJRoomSet.OnPosLeave(pos);
  }
  ...
  public OnPosContinueGame(pos) {
		this.BDMJRoomPosMgr.OnPosContinueGame(pos);
		this.BDMJRoomSet.OnPosContinueGame(pos);
  }
  ...
  public OnSetEnd(setEnd) {
		this.BDMJRoomSet.OnSetEnd(setEnd);
		this.BDMJRoomPosMgr.OnSetEnd(setEnd);
  }
  ...
  public OnStartVoteDissolve(createPos, endSec) {
		let posAgreeList = [];
		for(let index=0; index<this.ShareDefine.BDMJRoomJoinCount; index++){
			if(index == createPos){
				posAgreeList.push(1);
			}
			else{
  ...
  public OnDeleteOutCard(cardID) {
		...
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("OnDeleteOutCard not BDMJRoomState_Playing");
			return -1;
		}
  ...
  public GetRoomSet() {
		...
		return this.BDMJRoomSet
  }

  public GetRoomPosMgr() {
		return this.BDMJRoomPosMgr
  }
	//客户端玩家是否是开房人
  ...
  public GetActionEndTick(pos) {
		//不是牌局进行中
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("GetActionEndTick not BDMJRoomState_Playing");
			return 0
		}
		...
		let endTick = startWaitSec*1000 + this.PropertyInfo["BDMJRoom_WaitTick"];
  ...
  public GetClientPlayerSetPos() {
		...
		let pos = this.BDMJRoomPosMgr.GetClientPos();
		if(pos < 0){
			this.ErrLog("GetClientPlayerSetPos not enter room");
			return
		}
		let setPos = this.BDMJRoomSet.GetSetPosByPos(pos);
		if(!setPos){
  ...
  public GetClientPlayerInfo() {
		...
		let playerInfo = this.BDMJRoomPosMgr.GetPlayerInfoByPos(pos);
		if(!playerInfo){
