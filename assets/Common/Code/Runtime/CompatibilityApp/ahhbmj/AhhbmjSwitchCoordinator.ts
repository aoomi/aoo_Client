...
import { AhhbmjSceneBootstrap, type AhhbmjViewMode } from './AhhbmjSceneBootstrap';

/** Native handoff coordinator for backend game 376 (Jiangxi Ji'an Anfu Mahjong). */
export class AhhbmjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AhhbmjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 376 || handoff.gameName.toLowerCase() === 'ahhbmj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AhhbmjViewMode = '2d'): Promise<void> {
        if (this.switching || this.bootstrap || !this.accepts(handoff)) return;
        const roomId = Number(handoff.roomId ?? handoff.roomID ?? 0);
        if (!Number.isSafeInteger(roomId) || roomId <= 0) { handoff.cancel(); this.message('安徽淮北麻将房间数据无效'); return; }
        this.switching = true; this.source = handoff;
        try {
            if (!handoff.consume()) throw new Error('安徽淮北麻将切服票据已失效');
            const gameClient = createOwnedGameClient(); this.gameClient = gameClient;
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'ahhbmj');
            const host = new Node('AHHBMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AhhbmjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-ahhbmj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-ahhbmj-room-ready', { roomId, handoff, viewMode });
        } catch (error: unknown) { this.message(error instanceof Error ? error.message : '进入安徽淮北麻将失败'); await this.recoverHall(); }
