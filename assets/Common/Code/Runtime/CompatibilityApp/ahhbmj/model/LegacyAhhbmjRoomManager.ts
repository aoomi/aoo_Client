...
export class LegacyAhhbmjRoomManager {
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_ShowGenZhuang", this.OnPack_GenZhuang, this);

		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_ShowSJD", this.OnPack_DaoPai, this);
  ...
  public OnPack_PiaoFen(serverPack) {
		console.log("有玩家买马或双股通知 OnPack_PiaoFen", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_PiaoFen", serverPack);
  }
	//倒牌
  public OnPack_DaoPai(serverPack) {
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_DaoPai", serverPack);
  }
	//跟庄
