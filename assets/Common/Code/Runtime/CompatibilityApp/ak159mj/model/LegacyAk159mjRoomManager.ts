...
export class LegacyAk159mjRoomManager {
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_XiaPao", this.OnPack_XiaPao, this);		// 下跑
  ...
  public OnPack_XiaPao(serverPack) {
		console.log("set 下跑 OnPack_XiaPao", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_XiaPao", serverPack);
  }
  ...
  public OnPack_PosGetCard(serverPack) {
		...
		let liuPaiSize = serverPack["liuPaiSize"];
		...
		let oldLiuPaiSize = roomSet.GetRoomSetProperty("liuPaiSize");
		let isNormal = true;
		...
		if (roomSet.OnPosGetCard(pos, setPos, normalMoCnt, gangMoCnt, liuPaiSize)) {
  ...
  public SendPiaoFen(piaoFen) {
	...
	//下跑请求
  public SendXiaPao(piaoHua) {
		console.log("下跑请求 SendXiaPao", piaoHua);
