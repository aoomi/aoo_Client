...
const modalPaths: Record<ModalName, string> = {
    chat: 'native-ui/game/ak159mj/resources/ui/ak159mj_UIChat',
    settings: 'native-ui/game/ak159mj/resources/ui/ak159mj_UISetting02',
    dissolve: 'native-ui/game/ak159mj/resources/ui/ak159mj_UIMessage02',
};

/** Binds migrated table/menu prefabs to their original AK159MJ protocols. */
export class Ak159mjTableController {
    ...
    public constructor(
        ...
        private readonly runtime: Ak159mjRuntime,
        private readonly message: (text: string) => void,
    ) {
        this.bindTableButtons();
        this.root.on('legacy-ak159mj-event', this.onRoomEvent, this);
    }

    public destroy(): void {
        this.disposed = true;
        this.root.off('legacy-ak159mj-event', this.onRoomEvent, this);
        this.closeModal();
    }
    ...
    private bindTableButtons(): void {
        ...
        this.walk(this.root, (node) => {
            ...
            const xiaPao = /^btn_piaofen(\d+)$/.exec(node.name);
            if (xiaPao) this.bind(node, () => { void this.runtime.chooseXiaPao(Number(xiaPao[1])).catch(this.report('飘分失败')); });
        });
    }
    ...
    private onRoomEvent(payload: unknown): void {
        ...
        if (event.endsWith('startvotedissolve') || event.endsWith('posdealvote')) {
            ...
            this.root.emit('legacy-ak159mj-chat-message', packet.body);
        }
    }
    ...
    private bindSettings(modal: Node): void {
        ...
        for (const name of ['sd_yinyue', 'sd_yinxiao']) {
            ...
            slider?.node.on('slide', () => this.root.emit('legacy-ak159mj-volume', { name, value: slider.progress }), this);
        }
        const viewButtons = ['btn_2d', 'btn_XY', 'btn_WZ', 'btn_YF'];
        for (const name of viewButtons) {
            this.bindNamed(modal, name, () => this.root.emit('legacy-ak159mj-view-mode-request', name.slice(4).toLowerCase()));
        }
    }
