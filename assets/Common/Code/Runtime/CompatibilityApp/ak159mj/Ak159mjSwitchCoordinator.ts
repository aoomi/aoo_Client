...
import { Ak159mjSceneBootstrap, type Ak159mjViewMode } from './Ak159mjSceneBootstrap';

/** Native handoff coordinator for backend game 254 (Jiangxi Ji'an Anfu Mahjong). */
export class Ak159mjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: Ak159mjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 254 || handoff.gameName.toLowerCase() === 'ak159mj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: Ak159mjViewMode = '2d'): Promise<void> {
        if (this.switching || this.bootstrap || !this.accepts(handoff)) return;
        const roomId = Number(handoff.roomId ?? handoff.roomID ?? 0);
        if (!Number.isSafeInteger(roomId) || roomId <= 0) { handoff.cancel(); this.message('陕西安康159麻将房间数据无效'); return; }
        this.switching = true; this.source = handoff;
        try {
            if (!handoff.consume()) throw new Error('陕西安康159麻将切服票据已失效');
            const gameClient = createOwnedGameClient(); this.gameClient = gameClient;
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'ak159mj');
            const host = new Node('AK159MJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(Ak159mjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-ak159mj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-ak159mj-room-ready', { roomId, handoff, viewMode });
        } catch (error: unknown) { this.message(error instanceof Error ? error.message : '进入陕西安康159麻将失败'); await this.recoverHall(); }
