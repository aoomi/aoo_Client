...
import type { Ak159mjRuntime } from './Ak159mjRuntime';
import { Ak159mjPlayStateController, type Ak159mjPlaySnapshot } from './Ak159mjPlayStateController';
import type { Ak159mjViewMode } from './Ak159mjSceneBootstrap';
import { Ak159mjTableController } from './Ak159mjTableController';
...
const textureFolder: Record<Ak159mjViewMode, string> = {
...
/** Native Creator 3.8.8 binder shared by all four migrated AK159MJ table prefabs. */
@ccclass('Ak159mjPlayView')
export class Ak159mjPlayView extends Component {
    private runtime: Ak159mjRuntime | null = null;
    private state: Ak159mjPlayStateController | null = null;
    private viewMode: Ak159mjViewMode = '2d';
    private generation = 0;
    private resultNode: Node | null = null;
    private table: Ak159mjTableController | null = null;

    public initialize(runtime: Ak159mjRuntime, viewMode: Ak159mjViewMode): void {
        this.runtime = runtime;
        this.viewMode = viewMode;
        this.state = new Ak159mjPlayStateController(runtime);
        this.table = new Ak159mjTableController(this.node, runtime, (message) => this.node.emit('legacy-ak159mj-message', message));
        this.bindOperationButtons();
        this.node.on('legacy-ak159mj-event', this.onRoomEvent, this);
    }
    ...
    protected override onDestroy(): void {
        this.generation += 1;
        this.node.off('legacy-ak159mj-event', this.onRoomEvent, this);
    ...
    private async render(snapshot: Ak159mjPlaySnapshot): Promise<void> {
    ...
    private async showResult(phase: 'settled' | 'ended'): Promise<void> {
        ...
        const path = phase === 'ended'
            ? `native-ui/game/ak159mj/resources/game/AK159MJ/base/ak159mj_UIMJResultDetail${suffix}`
            : `native-ui/game/ak159mj/resources/game/AK159MJ/base/ak159mj_UIMJWinLost${suffix}`;
    ...
    private loadCardFrame(cardId: number): Promise<SpriteFrame> {
        ...
        const path = `legacy-ui/ak159mj-source/${textureFolder[this.viewMode]}/hh_face_li_${name}/spriteFrame`;
