...
import { AymjSceneBootstrap, type AymjViewMode } from './AymjSceneBootstrap';

/** Native handoff coordinator for backend game 80 (Jiangxi Ji'an Anfu Mahjong). */
export class AymjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AymjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 80 || handoff.gameName.toLowerCase() === 'aymj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AymjViewMode = '2d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'aymj');
            const host = new Node('AYMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AymjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-aymj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-aymj-room-ready', { roomId, handoff, viewMode });
