...
import type { AymjRuntime } from './AymjRuntime';
import { AymjPlayStateController, type AymjPlaySnapshot } from './AymjPlayStateController';
import type { AymjViewMode } from './AymjSceneBootstrap';
import { AymjTableController } from './AymjTableController';
...
const textureFolder: Record<AymjViewMode, string> = {
...
/** Native Creator 3.8.8 binder shared by all four migrated AYMJ table prefabs. */
@ccclass('AymjPlayView')
export class AymjPlayView extends Component {
    private runtime: AymjRuntime | null = null;
    private state: AymjPlayStateController | null = null;
    private viewMode: AymjViewMode = '2d';
    private generation = 0;
    private resultNode: Node | null = null;
    private table: AymjTableController | null = null;

    public initialize(runtime: AymjRuntime, viewMode: AymjViewMode): void {
        this.runtime = runtime;
        this.viewMode = viewMode;
        this.state = new AymjPlayStateController(runtime);
        this.table = new AymjTableController(this.node, runtime, (message) => this.node.emit('legacy-aymj-message', message));
        this.bindOperationButtons();
        this.node.on('legacy-aymj-event', this.onRoomEvent, this);
    }
    ...
    protected override onDestroy(): void {
        this.generation += 1;
        this.node.off('legacy-aymj-event', this.onRoomEvent, this);
    ...
    private async render(snapshot: AymjPlaySnapshot): Promise<void> {
    ...
    private async showResult(phase: 'settled' | 'ended'): Promise<void> {
        ...
        const path = phase === 'ended'
            ? `native-ui/game/aymj/resources/game/AYMJ/base/aymj_UIMJResultDetail${suffix}`
            : `native-ui/game/aymj/resources/game/AYMJ/base/aymj_UIMJWinLost${suffix}`;
    ...
    private loadCardFrame(cardId: number): Promise<SpriteFrame> {
        ...
        const path = `legacy-ui/aymj-source/${textureFolder[this.viewMode]}/hh_face_li_${name}/spriteFrame`;
