// @ts-nocheck
// Generated from the Creator 2.2.2 AYMJ room model; method bodies and protocol semantics are preserved.
export type LegacyAymjAppContext = Record<string, any>;
export class LegacyAymjRoomManager {
  public Log: (...args: any[]) => void = () => undefined;
  public ErrLog: (...args: any[]) => void = () => undefined;
  public constructor(private readonly app: LegacyAymjAppContext) { this.Init(); }
  ...
  public Init() {
		...
		this.AYMJRoom = this.app[this.app.subGameName.toUpperCase()+"Room"]();
		this.AYMJRoomSet = this.app[this.app.subGameName.toUpperCase()+"RoomSet"]();
		this.AYMJRoomPosMgr = this.app[this.app.subGameName.toUpperCase()+"RoomPosMgr"]();
		this.LocalDataManager = this.app.LocalDataManager();
		this.NetManager.RegNetPack("aymj.CAYMJCreateRoom", this.OnPack_CreateRoom, this);
		this.NetManager.RegNetPack("aymj.CAYMJGetRoomInfo", this.OnPack_GetRoomInfo, this);
		this.NetManager.RegNetPack("aymj.CAYMJRoomRecord", this.OnPack_RoomRecord, this);

		this.NetManager.RegNetPack("SAYMJ_CardReadyChg", this.OnPack_CardReadyChg, this);

		this.NetManager.RegNetPack("SAYMJ_SetStart", this.OnPack_SetStart, this);
		this.NetManager.RegNetPack("SAYMJ_SetEnd", this.OnPack_SetEnd, this);

		this.NetManager.RegNetPack("SAYMJ_StartRound", this.OnPack_StartRound, this);
		this.NetManager.RegNetPack("SAYMJ_Jin", this.OnPack_Jin, this);
		this.NetManager.RegNetPack("SAYMJ_PosGetCard", this.OnPack_PosGetCard, this);
		this.NetManager.RegNetPack("SAYMJ_PosOpCard", this.OnPack_PosOpCard, this);

		this.NetManager.RegNetPack("SAYMJ_RoomEnd", this.OnPack_RoomEnd, this);
		this.NetManager.RegNetPack("SAYMJ_RoomStart", this.OnPack_RoomStart, this);
		this.NetManager.RegNetPack("SAYMJ_reward", this.OnPack_Reward, this);
		this.NetManager.RegNetPack("SAYMJ_Applique", this.OnPack_Applique, this);
		this.NetManager.RegNetPack("SAYMJ_SQOpCard", this.OnPack_SQOpCard, this);
		this.NetManager.RegNetPack("SAYMJ_SetPosCard", this.OnPack_SetPosCard, this);

		this.NetManager.RegNetPack("SAYMJ_OpPiao", this.OnPack_OpPiao, this);

		this.NetManager.RegNetPack("SAYMJ_Promptly", this.OnPack_Promptly, this);
  ...
  public OnPack_SetStart(serverPack) {
		...
		this.AYMJRoom.OnSetStart(setInfo);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_SetStart", serverPack);
  ...
  public OnPack_SetEnd(serverPack) {
		...
		this.AYMJRoom.OnSetEnd(setEnd);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_SetEnd", serverPack);
  }
  ...
  public OnPack_Jin(serverPack) {
		...
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_Jin", serverPack);
  }
  ...
  public OnPack_StartRound(serverPack) {
		...
		this.AYMJRoom.GetRoomSet().OnStartRound(setRound);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_StartRound", serverPack);
  }
  ...
  public OnPack_PosGetCard(serverPack) {
		...
		let roomSet = this.AYMJRoom.GetRoomSet();
		...
		if (roomSet.OnPosGetCard(pos, setPos, normalMoCnt, gangMoCnt)) {
			...
			this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_PosGetCard", serverPack);
		} else {
  ...
  public OnPack_PosOpCard(serverPack) {
		...
		let roomID = serverPack["roomID"];
		if (this.AYMJRoom.UpdataInfo(serverPack)) {
			this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_PosOpCard", serverPack);
		} else {
  ...
  public OnPack_Promptly(serverPack) {
		...
		this.AYMJRoomPosMgr.UpdatePoint(setPosList);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_Promptly", serverPack);
  }
  ...
  public OnPack_SetPosCard(serverPack) {
		...
		this.AYMJRoomSet.InitSetPosList(setPosList);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_SetPosCard", serverPack);
  }
  ...
  public OnPack_Applique(serverPack) {
		...
		let roomID = serverPack["roomID"];
		if (this.AYMJRoom.UpdataInfo(serverPack)) {
			this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_Applique", serverPack);
		} else {
  ...
  public OnPack_SQOpCard(serverPack) {
		...
		let setRound = serverPack["room_SetWait"];
		this.AYMJRoom.GetRoomSet().OnStartRound(setRound);
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_SQOpCard", serverPack);
  }
  ...
  public OnPack_RoomEnd(serverPack) {
		...
		this.AYMJRoom.OnRoomEnd(serverPack);
  ...
  public OnPack_RoomRecord(serverPack) {
		...
		this.AYMJRoom.RoomRecord(records);
  ...
  public OnPack_CardReadyChg(serverPack) {
		this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_EVT_Card_Ready");
  ...
  public GetEnterRoom() {
		...
		return this.AYMJRoom;
  }

	//删除打出的卡牌
  public DeleteOutCard(cardID) {
		let findPos = this.AYMJRoom.OnDeleteOutCard(cardID);
		//如果找到需要删除的卡牌
		if (findPos != -1) {
			this.app[this.app.subGameName + "Client"].OnEvent("AYMJ_DeleteOutCard", {"pos": findPos});
		}
  }
  ...
  public SendCreateRoom(sendPack) {
		...
		this.NetManager.requestV2("aymj.CAYMJCreateRoom", sendPack);
  }
	//获取房间信息
  public SendGetRoomInfo(roomID, callback) {
		console.log("获取房间信息 SendGetRoomInfo", roomID, callback);
		this.NetManager.requestV2("aymj.CAYMJGetRoomInfo", {"roomID": roomID}, function (success) {
  ...
  public SendPosAction(cardID, opType) {
		...
		this.NetManager.requestV2("aymj.CAYMJOpCard", sendPack);
		...
		if (opType == this.ShareDefine.OpType_Out) {
			if (is3DShow == 1) {
				this.FormManager.GetFormComponentByFormName("game/AYMJ/ui/UIAYMJPlay").Pre_OutCard(cardID);
			} else if (is3DShow == 2) {
				this.FormManager.GetFormComponentByFormName("game/AYMJ/ui/UIAYMJXYPlay").Pre_OutCard(cardID);
			} else {
				this.FormManager.GetFormComponentByFormName("game/AYMJ/ui/UIAYMJ2DPlay").Pre_OutCard(cardID);
			}
		}
  }
	//不抢金不三金倒
  public SendSqPass(cardID, opType) {
		...
		this.NetManager.requestV2("AYMJ.CAYMJSQOpCard", sendPack);
  }
	//退出房间
  ...
  public SendRoomRecord(roomID) {
		...
		this.NetManager.requestV2("AYMJ.CAYMJRoomRecord", {"roomID": roomID});
  }
	//请求飘分封包
  public SendPiao(piao) {
		...
		let roomID = this.enterRoomID;
		let self = this;
        this.NetManager.requestV2("aymj.CAYMJOpPiao", {"roomID":roomID, "value":piao},function(success){},function(error){
        	self.ErrLog("aymj.CAYMJOpPiao.error:%s", error);
        });
  }
}
