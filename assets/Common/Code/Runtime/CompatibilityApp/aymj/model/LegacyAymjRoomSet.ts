...
export class LegacyAymjRoomSet {
  ...
  public Init() {
		...
		for (let posID = 0; posID < this.ShareDefine.AYMJRoomJoinCount; posID++) {
			this.cacheSetPosDict[posID] = this.app.AYMJSetPos();
		}
		this.OnReload();
  ...
  public OnPosOpCard(serverPack) {
		...
		let clientPos = this.AYMJRoomPosMgr.GetClientPos();
		if (clientPos == pos) {
			this.dataInfo["setRound"]["opPosList"] = [];//扣牌却换场景按钮会重复出现
		}
  ...
  public PreSetShouCard(pos, cardID) {
		...
		let dingQue = setPos.GetSetPosProperty("dingQue");
		let num = this.ShareDefine.AYMJDingQue[dingQue];
		let jin1=this.get_jin1();
		let jin2=this.get_jin2();
		//手牌刷新
		// if(Math.floor(jin1/100)==Math.floor(cardID/100)){
		// 	//打金了，需要获取是头搭二搭还是被搭
		// 	let forecastOutCard=setPos.GetSetPosProperty("forecastOutCard");
		// 	let daList=setPos.GetSetPosProperty("daList");
		// 	if(forecastOutCard=='TouDa'){
	 //             daList[0].push(cardID);
	 //        }else if(forecastOutCard=='ErDa'){
	 //             daList[1].push(cardID);
	 //        }else if(forecastOutCard=='PengDa'){
	 //             daList[2].push(cardID);
	 //        }
	 //        setPos.SetDataInfo('daList',daList);
		// }
		if(handCard>0 && handCard==cardID){
			...
			setPos.SetDataInfo('handCard',0);
				...
				for(let i=0;i<count;i++){
					if(Math.floor(shouCard[i]/1000)==num){
				...
				count=newShouCard.length;
				for(let i=0;i<count;i++){
					oldShouCard.push(newShouCard[i]);
				}
				setPos.SetDataInfo('shouCard',oldShouCard);
		}
  }
	//删除掉已经打出去的牌
  ...
  public GetHuCardTypeInfo(selfPos) {
		...
		for (var pos in this.allSetPosDict) {
			...
			let huMap = setPos.GetSetPosProperty("huMap");
			let handCard = setPos.GetSetPosProperty("handCard");
			if(!huMap["Not"]){
				if ((handCard > 1000 && handCard <5000 ) && outCardIDList.indexOf(handCard) == -1){
					outCardIDList = outCardIDList.concat(handCard);
				}
			}
		}
