...
export class LegacyAymjRoom {
  ...
  public OnReload() {
		...
		this.AYMJRoomPosMgr.OnReload();
		this.AYMJRoomSet.OnReload();
  }
  ...
  public OnInitRoomData(serverPack) {
		...
		this.AYMJRoomPosMgr.OnInitRoomPosData(roomPosInfoList);

		let setInfo = serverPack["set"];
		this.AYMJRoomSet.OnInitRoomSetData(setInfo);
		//初始化金
  ...
  public OnPosLeave(pos) {
		this.AYMJRoomPosMgr.OnPosLeave(pos);
		this.AYMJRoomSet.OnPosLeave(pos);
  }
  ...
  public OnPosContinueGame(pos) {
		this.AYMJRoomPosMgr.OnPosContinueGame(pos);
		this.AYMJRoomSet.OnPosContinueGame(pos);
  }
  ...
  public OnSetEnd(setEnd) {
		this.AYMJRoomSet.OnSetEnd(setEnd);
		this.AYMJRoomPosMgr.OnSetEnd(setEnd);
  }
  ...
  public OnStartVoteDissolve(createPos, endSec) {
		let posAgreeList = [];
		for(let index=0; index<this.ShareDefine.AYMJRoomJoinCount; index++){
			if(index == createPos){
				posAgreeList.push(1);
			}
			else{
  ...
  public OnDeleteOutCard(cardID) {
		...
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("OnDeleteOutCard not AYMJRoomState_Playing");
			return -1;
		}
  ...
  public GetRoomSet() {
		...
		return this.AYMJRoomSet
  }

  public GetRoomPosMgr() {
		return this.AYMJRoomPosMgr
  }
	//客户端玩家是否是开房人
  ...
  public GetActionEndTick(pos) {
		//不是牌局进行中
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("GetActionEndTick not AYMJRoomState_Playing");
			return 0
		}
		...
		let endTick = startWaitSec*1000 + this.PropertyInfo["AYMJRoom_WaitTick"];
  ...
  public GetClientPlayerSetPos() {
		...
		let pos = this.AYMJRoomPosMgr.GetClientPos();
		if(pos < 0){
			this.ErrLog("GetClientPlayerSetPos not enter room");
			return
		}
		let setPos = this.AYMJRoomSet.GetSetPosByPos(pos);
		if(!setPos){
  ...
  public GetClientPlayerInfo() {
		...
		let playerInfo = this.AYMJRoomPosMgr.GetPlayerInfoByPos(pos);
		if(!playerInfo){
