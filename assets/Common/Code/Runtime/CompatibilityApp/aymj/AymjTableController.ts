...
const modalPaths: Record<ModalName, string> = {
    chat: 'native-ui/game/aymj/resources/ui/aymj_UIChat',
    settings: 'native-ui/game/aymj/resources/ui/aymj_UISetting02',
    dissolve: 'native-ui/game/aymj/resources/ui/aymj_UIMessage02',
};

/** Binds migrated table/menu prefabs to their original AYMJ protocols. */
export class AymjTableController {
    ...
    public constructor(
        ...
        private readonly runtime: AymjRuntime,
        private readonly message: (text: string) => void,
    ) {
        this.bindTableButtons();
        this.root.on('legacy-aymj-event', this.onRoomEvent, this);
    }

    public destroy(): void {
        this.disposed = true;
        this.root.off('legacy-aymj-event', this.onRoomEvent, this);
        this.closeModal();
    }
    ...
    private onRoomEvent(payload: unknown): void {
        ...
        if (event.endsWith('startvotedissolve') || event.endsWith('posdealvote')) {
            ...
            this.root.emit('legacy-aymj-chat-message', packet.body);
        }
    }
    ...
    private bindSettings(modal: Node): void {
        ...
        for (const name of ['sd_yinyue', 'sd_yinxiao']) {
            ...
            slider?.node.on('slide', () => this.root.emit('legacy-aymj-volume', { name, value: slider.progress }), this);
        }
        const viewButtons = ['btn_2d', 'btn_XY', 'btn_WZ', 'btn_YF'];
        for (const name of viewButtons) {
            this.bindNamed(modal, name, () => this.root.emit('legacy-aymj-view-mode-request', name.slice(4).toLowerCase()));
        }
    }
