...
export type AymjPlayPhase = 'waiting' | 'playing' | 'settled' | 'ended';

export interface AymjPlaySnapshot {
    phase: AymjPlayPhase;
...
export class AymjPlayStateController {
    private phase: AymjPlayPhase = 'waiting';
    private hand: number[] = [];
    private availableOperations: number[] = [];
    private result: unknown = null;

    public constructor(private readonly runtime: AymjRuntime) {}

    public refreshFromRoom(): AymjPlaySnapshot {
    ...
    public consume(event: string, body: unknown): AymjPlaySnapshot {
    ...
    private snapshot(): AymjPlaySnapshot {
        return {
            phase: this.phase,
            hand: [...this.hand],
