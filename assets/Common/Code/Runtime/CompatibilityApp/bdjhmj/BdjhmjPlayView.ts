...
import type { BdjhmjRuntime } from './BdjhmjRuntime';
import { BdjhmjPlayStateController, type BdjhmjPlaySnapshot } from './BdjhmjPlayStateController';
import type { BdjhmjViewMode } from './BdjhmjSceneBootstrap';
import { BdjhmjTableController } from './BdjhmjTableController';
...
const textureFolder: Record<BdjhmjViewMode, string> = {
...
/** Native Creator 3.8.8 binder shared by all four migrated BDJHMJ table prefabs. */
@ccclass('BdjhmjPlayView')
export class BdjhmjPlayView extends Component {
    private runtime: BdjhmjRuntime | null = null;
    private state: BdjhmjPlayStateController | null = null;
    private viewMode: BdjhmjViewMode = '2d';
    private generation = 0;
    private resultNode: Node | null = null;
    private table: BdjhmjTableController | null = null;

    public initialize(runtime: BdjhmjRuntime, viewMode: BdjhmjViewMode): void {
        this.runtime = runtime;
        this.viewMode = viewMode;
        this.state = new BdjhmjPlayStateController(runtime);
        this.table = new BdjhmjTableController(this.node, runtime, (message) => this.node.emit('legacy-bdjhmj-message', message));
        this.bindOperationButtons();
        this.node.on('legacy-bdjhmj-event', this.onRoomEvent, this);
    }
    ...
    protected override onDestroy(): void {
        this.generation += 1;
        this.node.off('legacy-bdjhmj-event', this.onRoomEvent, this);
    ...
    private async render(snapshot: BdjhmjPlaySnapshot): Promise<void> {
    ...
    private async showResult(phase: 'settled' | 'ended'): Promise<void> {
        ...
        const path = phase === 'ended'
            ? `native-ui/game/bdjhmj/resources/game/BDJHMJ/base/bdjhmj_UIMJResultDetail${suffix}`
            : `native-ui/game/bdjhmj/resources/game/BDJHMJ/base/bdjhmj_UIMJWinLost${suffix}`;
    ...
    private loadCardFrame(cardId: number): Promise<SpriteFrame> {
        ...
        const path = `legacy-ui/bdjhmj-source/${textureFolder[this.viewMode]}/hh_face_li_${name}/spriteFrame`;
