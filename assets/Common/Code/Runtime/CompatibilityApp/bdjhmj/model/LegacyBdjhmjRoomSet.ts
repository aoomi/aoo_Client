...
export class LegacyBdjhmjRoomSet {
  ...
  public GetBiaoShiList() {
		let biaoShiList = this.dataInfo["biaoShiList"];
		if (0 == biaoShiList.length) {	// 初始化为默认值
  ...
  public SetBiaoShiList(biaoShiList) {
		this.dataInfo["biaoShiList"] = biaoShiList;
		return biaoShiList;
  }

  public ResetBiaoShiListWithFiledName(name) {
		this.dataInfo["biaoShiList"] = this.dataInfo["biaoShiList"] || [];
		let biaoShiList = this.dataInfo["biaoShiList"];
		this.RoomPosMgr = this.app[this.app.subGameName.toUpperCase() + "RoomPosMgr"]();
		let count = this.RoomPosMgr.GetRoomPlayerCount();
		for (let i = 0; i < count; i++) {
			biaoShiList[i] = biaoShiList[i] || {};
			biaoShiList[i][name] = -1;
		}
  ...
  public ReSetPiaoFenList() {
		this.dataInfo["piaoFenList"] = [];
		this.RoomPosMgr = this.app[this.app.subGameName.toUpperCase() + "RoomPosMgr"]();
		let count = this.RoomPosMgr.GetRoomPlayerCount();
		for (let i = 0; i < count; i++) {
			this.dataInfo["piaoFenList"][i] = -1;
		}

		return this.dataInfo["piaoFenList"];
  }

  public GetMaiList() {
		let maiList = this.dataInfo["maiList"];
		if (0 == maiList.length) {	// 初始化为默认值
			this.RoomPosMgr = this.app[this.app.subGameName.toUpperCase() + "RoomPosMgr"]();
			let count = this.RoomPosMgr.GetRoomPlayerCount();
			for (let i = 0; i < count; i++) {
				maiList[i] = -1;
  ...
  public SetMaiList(maiList) {
		this.dataInfo["maiList"] = maiList;
  }

  public ReSetMaiList() {
		this.dataInfo["maiList"] = [];
		this.RoomPosMgr = this.app[this.app.subGameName.toUpperCase() + "RoomPosMgr"]();
		let count = this.RoomPosMgr.GetRoomPlayerCount();
		for (let i = 0; i < count; i++) {
			this.dataInfo["maiList"][i] = -1;
		}

		return this.dataInfo["maiList"];
  }
  ...
  public GetHuCardTypeInfo(selfPos) {
		...
		for (var pos in this.allSetPosDict) {
			...
			for (let index = 0; index < publicCount; index++) {
				...
				if (opType == this.app[this.app.subGameName + "_ShareDefine"]().OpType_AnGang && selfPos != pos && this.app[this.app.subGameName + "_MJConfig"].GetAnGangShowType() == EnumType.EnumAnGangShowType.EnumAnGangShowType_SELF_SEE) {
		...
		for (var index = 0; index < count; index++) {
			...
			if (allCardTypeDict.hasOwnProperty(cardType)) {
				leftCount = cardTotal - allCardTypeDict[cardType];
			}
  ...
  public GetLeftCardTypeInfo(selfPos) {
		...
		for (var pos in this.allSetPosDict) {
			...
			for (let index = 0; index < publicCount; index++) {
				let publicInfoList = publicCardList[index];
				if (publicInfoList[0] == this.app[this.app.subGameName + "_ShareDefine"]().OpType_AnGang && this.app[this.app.subGameName + "_MJConfig"].GetAnGangShowType() == EnumType.EnumAnGangShowType.EnumAnGangShowType_SELF_SEE) {
					continue;
				}
				let cardIDList = publicInfoList.slice(3, publicInfoList.length);
				outCardIDList = outCardIDList.concat(cardIDList);
		...
		for (let index = 0; index < allCount; index++) {
			...
			if (isAdd && this.IsMinusOne(cardType)) {
				isAdd = false;
				allCardTypeDict[cardType] += 1;
			}
		}
  ...
  public IsMinusOne(cardType) {
		if (this.app[this.app.subGameName + "_MJConfig"].isMinusOneJin_1 && Math.floor(this.get_jin1() / 100) == cardType) {
			return true;
		}

		if (this.app[this.app.subGameName + "_MJConfig"].isMinusOneJin_2 && Math.floor(this.get_jin2() / 100) == cardType) {
			return true;
		}

		if (this.app[this.app.subGameName + "_MJConfig"].isMinusOneJinjin && Math.floor(this.get_jinJin() / 100) == cardType) {
			return true;
		}
