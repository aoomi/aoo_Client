...
export class LegacyBdjhmjRoomManager {
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_BiaoShi", this.OnPack_BiaoShi, this);
  ...
  public OnPack_PiaoFen(serverPack) {
		...
		let piaoFen = serverPack["piaoFen"];
		let pos = serverPack["pos"];

		let piaoFenList = this.RoomSet.GetPiaoFenList();
		piaoFenList[pos] = piaoFen;
		this.RoomSet.SetPiaoFenList(piaoFenList);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_PiaoFen", serverPack);
  }

	// 顠跑统一通知接口
  public OnPack_BiaoShi(serverPack) {
		console.log("顠跑统一通知接口 OnPack_BiaoShi", serverPack);
		let biaoShiList = serverPack["biaoShiList"];
		this.RoomSet.SetBiaoShiList(biaoShiList);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_BiaoShi", serverPack);
  }
  ...
  public OnPack_ChangeStatus(serverPack) {
		...
		let roomSet = this.Room.GetRoomSet();
		let curWaitingExType = serverPack["waitingExType"];	// 飘跑都为SetState_WaitingEx，具体顠或跑字段
		this.RoomSet.SetRoomSetProperty('waitingExType', curWaitingExType);
		this.RoomSet.SetBiaoShiList(serverPack["biaoShiList"]);
  ...
  public SendGetRoomInfo(roomID, callback, errorCb) {
		this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "GetRoomInfo", { "roomID": roomID }, (success) => {
  ...
  public SendPiaoFen(piaoFen) {
		console.log("飘请求 SendPiaoFen", piaoFen);
		this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "BiaoShi", {
			"roomID": this.enterRoomID,
			"choose": piaoFen
		}, (data) => {
	...
	// 记录玩家操作动作
  public SendDoOptype(opType) {
		if (opType == this.ShareDefine.OpType_Out) { //打牌
			return;
		}

		this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "DoOptype", {
			"roomID": this.enterRoomID,
			"opType": opType,
			"pos": this.RoomPosMgr.GetClientPos(),
		}, (data) => {
