...
export class LegacyBdjhmjRoomPositionManager {
  ...
  public OnSetEnd(setEnd) {
		...
		for (let pos in this.dataInfo) {
			...
			if (typeof (playerInfo.clubCent) != "undefined") {
				let roomClubCentSp = parseFloat(setEnd.posResultList[pos].roomClubCent).toFixed(2);
				//保留小数点后面两位
				playerInfo["clubCent"] = roomClubCentSp;
				this.dataInfo[pos]['clubCent'] = roomClubCentSp;
			}
