...
export class LegacyAsmjRoom {
  ...
  public OnStartChangePlaeryDissolve(createPos, playerNum, endSec) {
	...
	//位置更新解散
  public OnPack_ChangePlayerNumAgree(serverPack) {
		...
		this.app[this.app.subGameName + "Client"].OnEvent("PosChangePlayerDealVote", dissolveInfo);
  }
  ...
  public UpdataInfoJiPai(serverPack) {
		return this.GetRoomSet().UpdataInfoJiPai(serverPack);
  }
	//---------------------设置函数---------------------
	//删除已经打出的卡牌
  ...
  public GetActionEndTick(pos) {
		...
		let endTick = startWaitSec * 1000 + this.PropertyInfo["YXTDHRoom_WaitTick"];
