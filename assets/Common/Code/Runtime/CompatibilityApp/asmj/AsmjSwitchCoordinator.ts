...
import { AsmjSceneBootstrap, type AsmjViewMode } from './AsmjSceneBootstrap';

/** Native handoff coordinator for backend game 407 (Jiangxi Ji'an Anfu Mahjong). */
export class AsmjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AsmjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 407 || handoff.gameName.toLowerCase() === 'asmj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AsmjViewMode = '3d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'asmj');
            const host = new Node('ASMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AsmjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-asmj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-asmj-room-ready', { roomId, handoff, viewMode });
