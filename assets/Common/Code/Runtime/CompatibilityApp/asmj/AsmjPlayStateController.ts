...
export type AsmjPlayPhase = 'waiting' | 'playing' | 'settled' | 'ended';

export interface AsmjPlaySnapshot {
    phase: AsmjPlayPhase;
...
export class AsmjPlayStateController {
    private phase: AsmjPlayPhase = 'waiting';
    private hand: number[] = [];
    private availableOperations: number[] = [];
    private result: unknown = null;

    public constructor(private readonly runtime: AsmjRuntime) {}

    public refreshFromRoom(): AsmjPlaySnapshot {
    ...
    public consume(event: string, body: unknown): AsmjPlaySnapshot {
    ...
    private snapshot(): AsmjPlaySnapshot {
        return {
            phase: this.phase,
            hand: [...this.hand],
