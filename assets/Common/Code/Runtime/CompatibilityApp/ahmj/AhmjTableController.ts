...
const modalPaths: Record<ModalName, string> = {
    chat: 'native-ui/game/ahmj/resources/ui/ahmj_UIChat',
    settings: 'native-ui/game/ahmj/resources/ui/ahmj_UISetting02',
    dissolve: 'native-ui/game/ahmj/resources/ui/ahmj_UIMessage02',
};

/** Binds migrated table/menu prefabs to their original AHMJ protocols. */
export class AhmjTableController {
    ...
    public constructor(
        ...
        private readonly runtime: AhmjRuntime,
        private readonly message: (text: string) => void,
    ) {
        this.bindTableButtons();
        this.root.on('legacy-ahmj-event', this.onRoomEvent, this);
    }

    public destroy(): void {
        this.disposed = true;
        this.root.off('legacy-ahmj-event', this.onRoomEvent, this);
        this.closeModal();
    }
    ...
    private onRoomEvent(payload: unknown): void {
        ...
        if (event.endsWith('startvotedissolve') || event.endsWith('posdealvote')) {
            ...
            this.root.emit('legacy-ahmj-chat-message', packet.body);
        }
    }
    ...
    private bindSettings(modal: Node): void {
        ...
        for (const name of ['sd_yinyue', 'sd_yinxiao']) {
            ...
            slider?.node.on('slide', () => this.root.emit('legacy-ahmj-volume', { name, value: slider.progress }), this);
        }
        const viewButtons = ['btn_2d', 'btn_XY', 'btn_WZ', 'btn_YF'];
        for (const name of viewButtons) {
            this.bindNamed(modal, name, () => this.root.emit('legacy-ahmj-view-mode-request', name.slice(4).toLowerCase()));
        }
    }
