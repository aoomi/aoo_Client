...
export class LegacyAhmjRoomManager {
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_Niao", this.OnPack_OpDaNiao, this);
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_FlipCards", this.OnPack_FlipCards, this);
  ...
  public OnPack_GetRoomInfo(serverPack) {
		...
		if (this.app[this.app.subGameName + "_SceneManager"]().GetSceneType() != this.app.subGameName + "Scene") {
			...
			if (lastIs3DShow == 1) {
				...
				this.app[this.app.subGameName + "_FormManager"]().ShowForm("game/" + this.app.subGameName.toUpperCase() + "/ui/UI" + this.app.subGameName.toUpperCase() + "KLPlay");
			}
		}
  }
	//--------------notify-----------------
  ...
  public OnPack_OpDaNiao(serverPack) {
		console.log("OnPack_OpDaNiao:", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_OpDaNiao", serverPack);
  }
	//摇杠通知
  public OnPack_FlipCards(serverPack) {
		console.log("OnPack_FlipCards:", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_FlipCards", serverPack);
  }
	//局内消息表情
  ...
  public SendTimeOutContinue() {
	...
	//发送位置执行指令
  public SendPosAction(cardID, opType, gangCardList = []) {
		...
		this.NetManager.requestV2(this.app.subGameName.toUpperCase() + ".C" + this.app.subGameName.toUpperCase() + "OpCard", sendPack, (data) => {
			...
			if (opType == this.ShareDefine.OpType_Out || opType == this.ShareDefine.OpType_BaoTing) { //打牌
				if (is3DShow == 1) {
					...
					this.FormManager.GetFormComponentByFormName("game/" + this.app.subGameName.toUpperCase() + "/ui/UI" + this.app.subGameName.toUpperCase() + "KLPlay").Pre_OutCard(cardID);
				}
			}
		}, (error) => {
  ...
  public SendRoomRecord(roomID) {
	...
	//请求打鸟
  public SendDaNiao(niao) {
		if(!this.enterRoomID){
			this.ErrLog("SendDaNioa not enterRoomID");
			return
		}
		let roomID = this.enterRoomID;
		let self = this;
        this.NetManager.requestV2(this.app.subGameName.toUpperCase() + ".C" + this.app.subGameName.toUpperCase() + "DaNiao", {"roomID":roomID, "niao":niao},function(success){
        	console.log("DaNiao请求成功 value:", niao);
        },function(error){
        	self.ErrLog("DaNiao.error:%s", error);
        });
  }
}
