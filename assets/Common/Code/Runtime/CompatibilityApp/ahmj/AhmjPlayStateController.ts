...
export type AhmjPlayPhase = 'waiting' | 'playing' | 'settled' | 'ended';

export interface AhmjPlaySnapshot {
    phase: AhmjPlayPhase;
...
export class AhmjPlayStateController {
    private phase: AhmjPlayPhase = 'waiting';
    private hand: number[] = [];
    private availableOperations: number[] = [];
    private result: unknown = null;

    public constructor(private readonly runtime: AhmjRuntime) {}

    public refreshFromRoom(): AhmjPlaySnapshot {
    ...
    public consume(event: string, body: unknown): AhmjPlaySnapshot {
    ...
    private snapshot(): AhmjPlaySnapshot {
        return {
            phase: this.phase,
            hand: [...this.hand],
