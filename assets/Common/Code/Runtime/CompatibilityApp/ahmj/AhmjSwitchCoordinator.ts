...
import { AhmjSceneBootstrap, type AhmjViewMode } from './AhmjSceneBootstrap';

/** Native handoff coordinator for backend game 174 (Jiangxi Ji'an Anfu Mahjong). */
export class AhmjSwitchCoordinator {
    private gameClient: ProtocolClient | null = null;
    private host: Node | null = null;
    private bootstrap: AhmjSceneBootstrap | null = null;
    ...
    public accepts(handoff: LegacyExternalSubgameHandoff): boolean {
        return Number(handoff.gameId) === 174 || handoff.gameName.toLowerCase() === 'ahmj';
    }
    public async enter(handoff: LegacyExternalSubgameHandoff, viewMode: AhmjViewMode = '2d'): Promise<void> {
        ...
        try {
            ...
            const role = await new LegacyRoleGateway(gameClient, handoff.gameServerUrl).resumeWithToken(this.account, handoff.gameToken, 'ahmj');
            const host = new Node('AHMJNativeSceneHost'); host.layer = this.lobbyNode.layer;
            (this.lobbyNode.parent ?? this.lobbyNode).addChild(host); this.host = host;
            const bootstrap = host.addComponent(AhmjSceneBootstrap); this.bootstrap = bootstrap;
            await bootstrap.startRoom(gameClient, { roomId, playerId: role.playerId, viewMode,
                onRoomReady: () => { this.lobbyNode.active = false; },
                onEvent: (event, body) => this.lobbyNode.emit('legacy-ahmj-event', { event, body }),
                onMessage: this.message, onExit: (reason) => { void this.leave(reason); } });
            this.lobbyNode.emit('legacy-ahmj-room-ready', { roomId, handoff, viewMode });
