// @ts-nocheck
// Generated from the Creator 2.2.2 HZMJ model. Method bodies and protocol semantics are preserved.
export type LegacyHzmjAppContext = Record<string, any>;

export class LegacyHzmjRoomManager {
  public Log: (...args: any[]) => void = () => undefined;
  public ErrLog: (...args: any[]) => void = () => undefined;

  public constructor(private readonly app: LegacyHzmjAppContext) {
    this.Init();
  }
  ...
  public Init() {
		...
		this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_StopTime", this.OnPack_StopTime, this);//停止房间倒计时
        ...
        this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_ClubCentThresholdNotEnough", this.OnPack_ClubCentThresholdNotEnough, this);
        this.NetManager.RegNetPack("S" + this.app.subGameName.toUpperCase() + "_ClubCentThresholdEnough", this.OnPack_ClubCentThresholdEnough, this);
  ...
  public OnPack_GetRoomInfo(serverPack) {
		...
		//大包分包默认版本
		// let version = serverPack["set"]["version"];//（int 类型，1：大包 2：分包）
		// let is3DShow = this.app.LocalDataManager().GetConfigProperty("SysSetting", this.app.subGameName+"_is3DShow");
		// if(typeof(is3DShow) == "undefined"){
		// 	if(version == 1){
		// 		this.app.LocalDataManager().SetConfigProperty("SysSetting", this.app.subGameName+"_is3DShow", 3);
		// 	}else{
		// 		this.app.LocalDataManager().SetConfigProperty("SysSetting", this.app.subGameName+"_is3DShow", 4);
		// 	}
		// }
		this[this.app.subGameName.toUpperCase()+"Room"].OnInitRoomData(serverPack);
		//进入打牌场景
		if (this.app[this.app.subGameName + "_SceneManager"]().GetSceneType()!="hzmjScene") {
			this.app[this.app.subGameName + "_SceneManager"]().LoadScene("hzmjScene");
		}else{
			let lastIs3DShow = this.app.LocalDataManager().GetConfigProperty("SysSetting", this.app.subGameName+"_is3DShow");
			if (lastIs3DShow == 1) {
				this.app[this.app.subGameName + "_FormManager"]().ShowForm('game/HZMJ/ui/UIHZMJPlay');
			}else if (lastIs3DShow == 2) {
				this.app[this.app.subGameName + "_FormManager"]().ShowForm('game/HZMJ/ui/UIHZMJXYPlay');
			}else if (lastIs3DShow == 3) {
				this.app[this.app.subGameName + "_FormManager"]().ShowForm('game/HZMJ/ui/UIHZMJWZPlay');
			}else if (lastIs3DShow == 4) {
				this.app[this.app.subGameName + "_FormManager"]().ShowForm('game/HZMJ/ui/UIHZMJYXPlay');
			}
		}
  }
	//--------------notify-----------------
	//停止房间倒计时
  public OnPack_StopTime(serverPack) {
		console.log("停止房间倒计时 OnPack_StopTime", serverPack);
		this.app[this.app.subGameName + "Client"].OnEvent(this.app.subGameName.toUpperCase() + "_StopTime", serverPack);
  }
  ...
  public OnPack_ClubCentEnough(serverPack) {
	...
	//俱乐部积分门槛不足时通知
  public OnPack_ClubCentThresholdNotEnough(serverPack) {
        this.app[this.app.subGameName + "Client"].OnEvent("ClubCentThresholdNotEnough", serverPack);
  }
  public OnPack_ClubCentThresholdEnough(serverPack) {
        this.app[this.app.subGameName + "Client"].OnEvent("ClubCentThresholdEnough", serverPack);
  }
	//玩家的俱乐部积分在游戏外被改变通知
  ...
  public SendCreateRoom(sendPack) {
	...
	//获取房间信息
  public SendGetRoomInfo(roomID, callback, errorCb) {
        this.NetManager.requestV2(this.app.subGameName + ".C" + this.app.subGameName.toUpperCase() + "GetRoomInfo", { "roomID": roomID }, function (success) {
  ...
  public SendPosAction(cardID, opType) {
		...
		if(opType==this.ShareDefine.OpType_Out){ //打牌
			if(is3DShow==1){
				...
				this.FormManager.GetFormComponentByFormName("game/"+this.app.subGameName.toUpperCase()+"/ui/UI"+this.app.subGameName.toUpperCase()+"YXPlay").Pre_OutCard(cardID);
			}else{
