...
export class LegacyHzmjRoom {
  ...
  public OnStartChangePlaeryDissolve(createPos,playerNum,endSec) {
		let posAgreeList = [];
		for(let index=0; index<this.ShareDefine.HZMJRoomJoinCount; index++){
			if(index == createPos){
				posAgreeList.push(1);
			}
			else{
  ...
  public OnDeleteOutCard(cardID) {
		...
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("OnDeleteOutCard not HZMJRoomState_Playing");
			return -1;
		}
  ...
  public GetActionEndTick(pos) {
		//不是牌局进行中
		if(this.dataInfo["state"] != this.ShareDefine.RoomState_Playing){
			this.ErrLog("GetActionEndTick not HZMJRoomState_Playing");
			return 0
		}
