...
const XQP_ROOM_BACKGROUND = 'Atlas/XqpRoomBackground/spriteFrame';
const XQP_CARD_FACES = 'Atlas/PokerFront_0Trends';
const XQP_COMMON_STATIC = 'Atlas/CxCommonStatic';
const XQP_BANKER_BADGE = 'icon_zhuang';
const XQP_BET_EFFECTS = Object.freeze({
    RAISE: { asset: 'Spine/Add/add', animation: 'ren_wu_jiangli_tishi_loop', scale: new Vec3(0.6, 0.6, 1), node: 'BetAnim_4' },
    ALL_IN: { asset: 'Spine/Allin/allin', animation: 'dz_mptx_huo', scale: new Vec3(1, 0.8, 1), node: 'BetAnim_5' },
});
...
export class CD299LandscapeRoomViewComponent extends Component implements CD299RoomView {
    ...
    private async openAddCentWindow(seat: number): Promise<void> {
        ...
        try {
            const accepted = await this.controller.sit(0);
            if (!accepted) throw new Error('[CD299] seat reservation returned no authoritative state');
            this.seatReservationPending = null;
            const reservedSeat = this.controller.authoritativeSeat();
            if (reservedSeat < 0) throw new Error('[CD299] seat reservation has no authoritative seat');
            ...
            scoreInput.on(Node.EventType.TOUCH_END, () => {
                console.info('[CD299] carry score editor requested', { roomId: this.roomId, seat: reservedSeat });
                void this.openCarryScoreNumpad(window).catch(error => console.error('[CD299] carry score editor failed', {
                    roomId: this.roomId, seat: reservedSeat, reason: error instanceof Error ? error.message : String(error),
                }));
            }, this);
            this.bindPopupButton(window.getChildByPath('Add/Btns/Sure'), () => {
                const carryScore = this.popupCarryScore(window);
                console.info('[CD299] add-cent confirmed', { roomId: this.roomId, seat: reservedSeat, carryScore });
                void this.controller?.sit(carryScore).then(() => {
                    this.reservedSeat = null;
                    this.closeAddCentWindow();
                });
            }, true);
