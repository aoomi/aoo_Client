...
export class CD299LandscapeRoomViewComponent extends Component implements CD299RoomView {
    ...
    public bindController(controller: CD299RuntimeController): () => void {
        ...
        this.bind(this.path('Btn/Preset/Preset_1'), () => controller.preset(1));
        this.bind(this.path('Btn/Preset/Preset_2'), () => controller.preset(2));
        this.bind(this.path('Btn/Bet/Btn_Drop'), () => this.submitBet('DROP', 0));
        this.bind(this.path('Btn/Bet/Btn_Follow/Btn_Follow'), () => this.submitBet('FOLLOW', 0));
        this.bind(this.path('Btn/Bet/Btn_Follow/Btn_Rest'), () => this.submitBet('REST', 0));
        this.bind(this.path('Btn/Bet/Btn_Add/Btn_AllIn'), () => this.submitBet('ALL_IN', 0));
        ...
        this.bind(this.path('Btn/Bet/Btn_Add/Add/Fast/Btn_Card_1/Btn_Select'), () => this.submitQuickRaise(0));
        this.bind(this.path('Btn/Bet/Btn_Add/Add/Fast/Btn_Card_2/Btn_Select'), () => this.submitQuickRaise(1));
        this.bind(this.path('Btn/Bet/Btn_Add/Add/Fast/Btn_Card_3/Btn_Select'), () => this.submitQuickRaise(2));
        this.bind(this.path('Btn/Split/Btn_Ensure'), () => this.submitSplit());
        this.bind(this.path('Btn/Split/Btn_Delay'), () => controller.delaySplit());
    ...
    public showHand(seat: number, cards: readonly number[], revealed: boolean): void {
        ...
        void Promise.all(cards.map(async (rawCard, index) => {
            ...
            if (seat === 0) {
                ...
                card.setPosition(index * 75, 0, index);
    ...
    public showCommitted(seat: number, value: number): void {
        const area = this.seat(seat).getChildByName('Bet');
    ...
    public showDropped(seat: number, dropped: boolean): void {
        const node = this.seat(seat).getChildByName('Dropped');
    ...
    public showSplit(seat: number, enabled: boolean): void {
        const node = this.seat(seat).getChildByName('Split');
        if (node) node.active = enabled;
    }
    ...
    public setActions(actions: Readonly<CD299Actions>): void {
        ...
        this.visible('Btn', actions.canPreset || actions.canBet || actions.canSplit);
        this.visible('Btn/Preset', actions.canPreset);
        this.visible('Btn/Bet', actions.canBet);
        this.visible('Btn/Bet/Btn_Drop', actions.canBet && actions.betActions.includes('DROP'));
        this.visible('Btn/Bet/Btn_Follow', actions.canBet
        ...
        this.visible('Btn/Bet/Btn_Follow/Btn_Follow', actions.canBet
            && actions.betActions.includes('FOLLOW') && !canRest);
        this.visible('Btn/Bet/Btn_Follow/Btn_Rest', canRest);
        const followLabel = this.path('Btn/Bet/Btn_Follow/Btn_Follow/Label')?.getComponent(Label);
    ...
    private hideAllActions(): void {
        const operateBtn = this.path('Btn');
        if (operateBtn) {
    ...
    private renderSplitSelection(): void {
        const cardsRoot = this.path('Btn/Split/Cards');
        if (!cardsRoot) return;
        for (let index = 0; index < 2; index += 1) {
            const slot = cardsRoot.getChildByName(`Btn_Card_${index}`);
            if (!slot) continue;
        ...
        const headLabel = this.path('Btn/Split/HeadType/Label')?.getComponent(Label);
    ...
    private async renderHead(seat: number, playerId: number | null): Promise<void> {
        ...
        if (!head) {
            ...
            mount.addChild(head);
            const source = head.getComponent(UITransform)?.contentSize;
            const target = mount.getComponent(UITransform)?.contentSize;
