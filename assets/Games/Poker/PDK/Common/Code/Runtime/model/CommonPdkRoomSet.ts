...
export class CommonPdkRoomSet {
	...
	public GetHandCard(){
		...
		if(!Array.isArray(posInfo)){
			throw new Error("CommonPdk 权威 posInfo 缺失");
		}
		for(let i = 0; i < posInfo.length; i++){
			if(posInfo[i].pid == this.app[this.app.subGameName+"_HeroManager"]().GetHeroProperty("pid")){
				if(!Array.isArray(posInfo[i].cards)){
					throw new Error("CommonPdk 本家手牌数组缺失");
				}
				return posInfo[i].cards;
			}
		}
		throw new Error("CommonPdk 本家座位未找到");
	}
}
