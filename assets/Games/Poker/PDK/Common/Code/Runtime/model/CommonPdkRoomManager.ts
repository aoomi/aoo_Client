// @ts-nocheck
// Generated from Creator 2.4.8 by tools/migrate-pdk-models.mjs.
// Legacy method bodies are preserved; only the BaseClass container is replaced.
export type CommonPdkAppContext = Record<string, any>;

export class CommonPdkRoomManager {
    public constructor(private readonly app: CommonPdkAppContext) {
	...
	public Init(){
		...
		//俱乐部积分通知
		this.NetManager.RegNetPack("CommonPdk_ClubCentNotEnough", this.OnPack_ClubCentNotEnough, this);
		this.NetManager.RegNetPack("CommonPdk_ClubCentEnough", this.OnPack_ClubCentEnough, this);
	...
	public OnPack_GetRoomInfo(serverPack){
		...
		// 其 sceneName 可能仍停留在 pdkScene。必须同时核对 Cocos 当前真实场景，
		// 否则第二次进入会误走 ShowForm，拿旧的已销毁节点并永久停在进度条。
		let isActualGameScene = this.app.__isActualGameScene();
		if (this.app[this.app.subGameName + "_SceneManager"]().GetSceneType()!="pdkScene" || !isActualGameScene) 
		{
			this.app[this.app.subGameName + "_SceneManager"]().LoadScene("pdkScene");
		}
		else{
			this.app[this.app.subGameName + "_FormManager"]().ShowForm('pdk/PDK_CommonRoom');
		}
	...
	public SendGetRoomInfo(roomID, callback){
		console.log('SendGetRoomInfo begin pdk')
