...
export class CommonPdkReplayController {
    ...
    private apply(frame: ReplayFrame): void {
        ...
        if (name.includes('_Config')) this.text('wanfa', body.roomType === 1 ? '俱乐部' : body.roomType === 4 ? '赛事' : '好友房');
    ...
    private async renderCards(path: string, values: number[]): Promise<void> {
        ...
        for (const value of sorted) await this.cards.create(parent, value);
    }
    ...
    private async changeSet(delta: number): Promise<void> {
        ...
        try {
            const response: any = await this.runtime.request('pdk.CommonPdkGetPlayBackCode', { roomId: this.roomId, tabId: target });
            this.tabId = target;
